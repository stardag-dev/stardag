...
Stardag's built-in filesystem targets (for, for example, local disk, S3 and Modal volumes) already implement complete async interfaces. Even if your task only implements the sync `def run(self)` method, the async implementation of targets can provide significant (order(s) of magnitude) speedup in traversing DAGs with a large number of dependencies.
