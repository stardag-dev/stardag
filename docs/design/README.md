...
| [executions-as-records.md](executions-as-records.md)                       | Why an execution gets a row rather than being reassembled from the event log in eight places, why several rows may be open for one task, and why the row carries no expiry of its own |
