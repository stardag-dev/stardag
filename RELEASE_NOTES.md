...
**Paired with `server-v0.5.0`, and the last release of the v1 line**; v2 follows as the next minor, a breaking release with its own migration notes. The server image also carries what v0.25.0's notes deferred to "the next server image": the `Stop running tasks` panel and the server half of the idempotent claim.
